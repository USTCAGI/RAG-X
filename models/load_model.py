...
def load_chat_model(model_name="gpt-4o", api_key=None, base_url=None, **kwargs):
...
def load_model(model_name="gpt-4o", api_key=None, base_url=None, **kwargs):
    if api_key is None and base_url is None:
        ...
        model = OpenAI(model_name=model_name, api_key=api_key, **kwargs)
    elif api_key is None and base_url is not None:
        model = OpenAI(model_name=model_name, base_url=base_url, **kwargs)
    else:
        model = OpenAI(model_name=model_name, api_key=api_key, base_url=base_url, **kwargs)
    return model

def load_chat_model_ollama(model_name="llama3", **kwargs):
