...
from models.mock_api.prompts import NER_FINANCE_PROMPT_TEMPLATE, NER_MUSIC_PROMPT_TEMPLATE, NER_MOVIE_PROMPT_TEMPLATE, NER_SPORTS_PROMPT_TEMPLATE, NER_OPEN_PROMPT_TEMPLATE
import re
import vllm
...
class MockAPI:
    def __init__(self, llm, terminators, tokenizer):
    ...
    def format_ner_prompt(self, query, domain):
        if domain == "finance":
            return NER_FINANCE_PROMPT_TEMPLATE.format(query=query)
        elif domain == "music":
            return NER_MUSIC_PROMPT_TEMPLATE.format(query=query)
        elif domain == "movie":
            return NER_MOVIE_PROMPT_TEMPLATE.format(query=query)
        elif domain == "sports":
            return NER_SPORTS_PROMPT_TEMPLATE.format(query=query)
        else:
            return NER_OPEN_PROMPT_TEMPLATE.format(query=query)
    
    def format_ner_prompts(self, queries, domains):
        prompts = []
        for query, domain in zip(queries, domains):
            prompts.append(self.format_ner_prompt(query, domain))
        return prompts
    ...
    #     final_prompts = self.format_ner_prompts(queries, domains)
    ...
    #             stop_token_ids=self.terminators,  # Token IDs at which to stop generation.
    #         ),
    ...
    #         results.append(self.ner_output_parser(output, domains[i]))
    #     return results
    ...
    def ner(self, queries, domains):
        ...
        for query, domain in  zip(queries, domains):
            ...
            if domain == "finance":
                prompt_template = ChatPromptTemplate.from_messages(
                    [("system", SYSTEM_PROMPT), ("user", NER_FINANCE_PROMPT_TEMPLATE)]
                )
            elif domain == "music":
                prompt_template = ChatPromptTemplate.from_messages(
                    [("system", SYSTEM_PROMPT), ("user", NER_MUSIC_PROMPT_TEMPLATE)]
                )
            elif domain == "movie":
                prompt_template = ChatPromptTemplate.from_messages(
                    [("system", SYSTEM_PROMPT), ("user", NER_MOVIE_PROMPT_TEMPLATE)]
                )
            elif domain == "sports":
                prompt_template = ChatPromptTemplate.from_messages(
                    [("system", SYSTEM_PROMPT), ("user", NER_SPORTS_PROMPT_TEMPLATE)]
                )
            else:
                prompt_template = ChatPromptTemplate.from_messages(
                    [("system", SYSTEM_PROMPT), ("user", NER_OPEN_PROMPT_TEMPLATE)]
                )
                
            self.llm_chain = prompt_template | self.llm | StrOutputParser()
            response = self.llm_chain.invoke({"query": query})

            # 解析输出并将结果添加到结果列表中
            output = response
            # print(response)
            results.append(self.ner_output_parser(output, domain))
    ...
    def get_movie_info(self, query, query_time, matched_entities):
        ...
        for movie_id in movie_ids:
            ...
            # else:
            #     info += f"#### Some information of {movie_info['title']} ({movie_info['original_title']})\n"
            info += f"#### Some information of {movie_info['title']}\n"
